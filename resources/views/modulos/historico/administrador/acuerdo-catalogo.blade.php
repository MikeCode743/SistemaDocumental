...
<html lang="en">
  ...
  <body>
    ...
    <script>
      ...
      const baseURLDelete = window.location.pathname+"/eliminar";

      @if (session()->has('notificacion'))
        Swal.fire({!! json_encode(session()->get('notificacion')) !!});
      @endif

      const datos = {!! json_encode($response) !!};
      ...
      document.addEventListener("alpine:init", () => {
        ...
        const defaultFormData = { id: "", nombre: "", descripcion: "" };
        ...
        Alpine.data("form", () => ({
          ...
          deleteItem(itemId) {
            ...
            const datosFiltrados = datos.find(({ id }) => id === itemId);
            this.formData = { ...datosFiltrados };
          },

          updateItem(itemId) {
            this.isCreateOrUpdateForm = true;
            const datosFiltrado = datos.find(({ id }) => id === itemId);
            this.formData = { ...datosFiltrado };
          },
